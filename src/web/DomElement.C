...
#define toChar(b) char(b)
                            ...
                            bool isExposed)
  {
    ...
    bool anchorClick = type() == DomElementType::A && eventName == WInteractWidget::CLICK_SIGNAL;
  ...
  {
    if (minMaxSizeProperties_ && app->environment().agent() == UserAgent::IE6)
    {
  ...
  void DomElement::processEvents(WApplication *app) const
  {
    ...
    if (keypress != eventHandlers_.end() && !keypress->second.jsCode.empty())
      Utils::access(self->eventHandlers_, S_keypress).jsCode = "if (" WT_CLASS ".isKeyPress(event)){" + Utils::access(self->eventHandlers_, S_keypress).jsCode + '}';
  }
                                   ...
                                   char delimiter)
  {
    ...
    out.pushEscape(delimiter == '\'' ? EscapeOStream::JsStringLiteralSQuote : EscapeOStream::JsStringLiteralDQuote);
    out << s;
    out.popEscape();
         ...
         j != properties_.end(); ++j)
    {
               ...
               (p < static_cast<unsigned int>(Property::LastPlusOne)))
      {
        ...
        else
        {
          if (!j->second.empty())
          {
            style << cssNames_[p - static_cast<unsigned int>(Property::StylePosition)]
                  << ':' << j->second << ';';
                          ...
                          bool openingTagOnly) const
  {
    ...
    EventHandlerMap::const_iterator clickEvent = eventHandlers_.find(WInteractWidget::CLICK_SIGNAL);

    bool needButtonWrap = (!app->environment().ajax() && (clickEvent != eventHandlers_.end()) && (!clickEvent->second.signalName.empty()) && (!app->environment().agentIsSpiderBot()));
   ...
   */
    if (needButtonWrap)
    {
      if (type_ == DomElementType::AREA || type_ == DomElementType::INPUT || type_ == DomElementType::SELECT)
        needButtonWrap = false;
       ...
       */
        ...
        {
          DomElement *self = const_cast<DomElement *>(this);
          self->setAttribute("href", app->url(app->internalPath()) + "&signal=" + clickEvent->second.signalName);
          needButtonWrap = false;
        }
      }
    ...
    if (needButtonWrap)
    {
      if (supportButton)
      {
        ...
        if (app->environment().agent() != UserAgent::Konqueror && !app->environment().agentIsWebKit() && !app->environment().agentIsIE())
    ...
    if (app->environment().ajax())
    {
           ...
           i != eventHandlers_.end(); ++i)
      {
        ...
        {
          if (globalUnfocused_ || (i->first == WInteractWidget::WHEEL_SIGNAL &&
                                   app->environment().agentIsIE() &&
                                   static_cast<unsigned int>(app->environment().agent()) >=
                                       static_cast<unsigned int>(UserAgent::IE9)))
     ...
     */
      if (!isSelfClosingTag(renderedType))
      {
        ...
        if (renderedType == DomElementType::DIV && app->environment().agent() == UserAgent::IE6 && innerHTML.empty() && childrenToAdd_.empty() && childrenHtml_.empty())
          out << "&nbsp;";
   ...
   */
    if ((app->environment().agentIsIE() || app->environment().agent() == UserAgent::Konqueror) && (type_ == DomElementType::TBODY || type_ == DomElementType::THEAD || type_ == DomElementType::TABLE || type_ == DomElementType::COLGROUP || type_ == DomElementType::TR || type_ == DomElementType::SELECT || type_ == DomElementType::TD || type_ == DomElementType::OPTGROUP))
      return false;
...
bool DomElement::containsElement(DomElementType type) const
{
                                 ...
                                 const std::string &domInsertJS)
  {
    ...
    if (app->environment().agentIsIE() && app->environment().agent() <= UserAgent::IE8 && type_ != DomElementType::TEXTAREA)
    {
      /*
  ...
  void DomElement::renderInnerHtmlJS(EscapeOStream &out, WApplication *app) const
  {
     ...
     */
      if ((type_ == DomElementType::DIV && app->environment().agent() == UserAgent::IE6) || !childrenToAdd_.empty() || !childrenHtml_.empty() || !innerHTML.empty())
      {
        declare(out);
        ...
        if (type_ == DomElementType::DIV && app->environment().agent() == UserAgent::IE6 && childrenToAdd_.empty() && innerHTML.empty() && childrenHtml_.empty())
          out << "&nbsp;";
  ...
  {
    return ((tag == "br") || (tag == "hr") || (tag == "img") || (tag == "area") || (tag == "col") || (tag == "input"));
  }
  ...
  bool DomElement::isSelfClosingTag(DomElementType element)
  {
    return ((element == DomElementType::BR)
            ...
            || (element == DomElementType::IMG) || (element == DomElementType::AREA) || (element == DomElementType::COL) || (element == DomElementType::INPUT));
  }
