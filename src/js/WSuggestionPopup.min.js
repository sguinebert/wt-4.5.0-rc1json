WT_DECLARE_WT_MEMBER(1,JavaScriptConstructor,"WSuggestionPopup",function(s,g,y,G,q,t,z,u){function c(a){return $(a).hasClass("Wt-suggest-onedit")||$(a).hasClass("Wt-suggest-dropdown")}function d(){return g.style.display!="none"}function j(a){g.style.display="block";e.positionAtWidget(g.id,a.id,e.Vertical);g.style.top=a.getBoundingClientRect().bottom+ window.scrollY + 'px';g.style.bottom='auto';}function h(a){a=e.target(a||window.event);if(!e.hasTag(a,"UL")){for(;a&&!e.hasTag(a,"LI");)a=a.parentNode;a&&p(a)}}function p(a){var b=a.firstChild.firstChild,i=e.getElement(f),l=b.innerHTML;b=b.getAttribute("sug");
...
a.offsetTop}$(".Wt-domRoot").add(g);g.wtObj=this;var r=this,e=s.WT,o=null,f=null,K=false,C=null,M=null,N=t,D=null,E=null,v=false;this.defaultValue=z;var A=null;this.showPopup=function(a){g.style.display="block";E=o=null;A=a.onkeydown;a.onkeydown=function(b){r.editKeyDown(this,b||window.event)}};this.editMouseMove=function(a,b){if(c(a))a.style.cursor=e.widgetCoordinates(a,b).x>a.offsetWidth-16?"default":""};this.showAt=function(a,b){m();f=a.id;v=true;r.refilter(b)};this.editClick=function(a,b){if(c(a))if(e.widgetCoordinates(a,
  b).x>a.offsetWidth-16)if(f!=a.id||!d())r.showAt(a,"");else{m();f=null}};this.editKeyDown = function (a, b) {if (!c(a)) return true; if (f != a.id) if ($(a).hasClass("Wt-suggest-onedit")) { f = a.id; v = false } else if ($(a).hasClass("Wt-suggest-dropdown") && b.keyCode == 40) { f = a.id; v = true } else { f = null; return true } var i = o ? e.getElement(o) : null;if(d()&&!i){var F=g.childNodes;if(b.keyCode==40||b.keyCode==34){i=F[0];if(i.style.display=='none')i=B(i,1);}if(b.keyCode==38||b.keyCode==33){i=F[F.length-1];if(i.style.display=='none')i=B(i,0);e.cancelEvent(b);}if(b.keyCode==13){m();const ke=new KeyboardEvent('keydown',{bubbles:false,cancelable:true,keyCode:13});e.getElement(f).dispatchEvent(ke);/*var ed=e.getElement(f);s.emit(g,"select",null,ed.id);*/}if(i){o=i.id;i.className="active";}return;} if(d()&&i)if(b.keyCode==13||b.keyCode==9){p(i);e.cancelEvent(b);setTimeout(function(){a.focus()},0);return false}else if(b.keyCode==40||b.keyCode==38||b.keyCode==34||b.keyCode==33){if(b.type.toUpperCase()==
"KEYDOWN"){K=true;e.cancelEvent(b,e.CancelDefaultAction)}if(b.type.toUpperCase()=="KEYPRESS"&&K==true){e.cancelEvent(b);return false}var l=i,F=b.keyCode==40||b.keyCode==34;b=b.keyCode==34||b.keyCode==33?g.clientHeight/i.offsetHeight:1;var n;for(n=0;l&&n<b;++n){var w=B(l,F);if(!w){l.className="";o=l=null;break;}l=w}if(l&&e.hasTag(l,"LI")){i.className="";l.className="active";o=l.id}return false}return b.keyCode!=13&&b.keyCode!=9};this.filtered=function(a,b){C=a;N=b;r.refilter(E)};this.refilter=function(a){if(f){var b=o?e.getElement(o):
