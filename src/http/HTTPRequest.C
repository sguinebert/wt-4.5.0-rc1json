...
namespace http
{
  namespace server
  {
    ...
    {
      //if(!resonseflushed_)
      //  std::cout << "HTTPRequest reset " << queryString() << std::endl;
      WebRequest::reset();

      reply_ = reply;
      entryPoint_ = entryPoint;

      //resonseflushed_ = false;
    }
    ...
    void HTTPRequest::flush(ResponseState state, const WriteCallback &callback)
    {
      ...
      if (state == ResponseState::ResponseDone)
        reply_.reset();
//std::cout << "HTTPRequest flush " << queryString() << std::endl;
      ptr->send(callback, state == ResponseState::ResponseDone);
      //resonseflushed_ = true;
    }
    ...
    {
      //std::cerr << "setRedirect " << url << std::endl;
    ...
    const std::string &HTTPRequest::pathInfo() const
    {
      ...
      //std::cerr << "pathInfo " << p->request().request_extra_path << std::endl;
    ...
    const std::string &HTTPRequest::remoteAddr() const
    {
      ...
      //std::cerr << "remoteAddr " << p->request().remoteIP << std::endl;
    ...
    const char *HTTPRequest::urlScheme() const
    {
      ...
      //std::cerr << "urlScheme " << p->request().urlScheme << std::endl;
    ...
    const std::vector<std::pair<std::string, std::string>> &HTTPRequest::urlParams() const
    {
      ...
      // for (auto e : p->request().url_params)
      //   std::cerr << "urlScheme " << e.first << " - " << e.second << std::endl;
