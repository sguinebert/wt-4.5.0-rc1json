  ...
  namespace server
  {
    ...
    bool RequestParser::doWebSocketHandshake00(const Request &req)
    {
      ...
      if (k1 && k2 && origin)
      {
        ...
        if (parseCrazyWebSocketKey(k1->value, n1) && parseCrazyWebSocketKey(k2->value, n2))
        {
                                         ...
                                         char *end)
    {
      ...
      case ws_start:
      {
        if (req.webSocketVersion != 0 && req.webSocketVersion != 7 && req.webSocketVersion != 8 && req.webSocketVersion != 13)
        {
        ...
        if (req.webSocketVersion == 0)
        {
          ...
          std::string location = std::string(req.urlScheme) + "://" + host->value.str() + req.request_path + "?" + req.request_query;
