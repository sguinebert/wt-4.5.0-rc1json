...
namespace http
{
  namespace server
  {
    ...
    void Server::start()
    {
          ...
          config_.parentPort() == -1)
      {
        ...
        long sslOptions = asio::ssl::context::default_workarounds | asio::ssl::context::no_sslv2 | asio::ssl::context::single_dh_use;
    ...
    defined(WT_ASIO_IS_STANDALONE_ASIO)
        ...
        if (!config_.sslCipherList().empty())
        {
          if (!SSL_CTX_set_cipher_list(native_ctx, config_.sslCipherList().c_str()))
          {
            throw Wt::WServer::Exception(
                "failed to select ciphers for cipher list " + config_.sslCipherList());
          }
        }
      ...
      if (config_.parentPort() != -1)
      {
        ...
        std::shared_ptr<asio::ip::tcp::socket> parentSocketPtr(new asio::ip::tcp::socket(wt_.ioService()));
        wt_.ioService().post(std::bind(&Server::startConnect, this, parentSocketPtr));
      }
    }
                                ...
                                const std::string &port)
    {
      ...
      if (config_.parentPort() == -1)
      {
        ...
        if (addresses.empty())
          throw Wt::WException(std::string("Could not bind to address ") + address + " port " + port + ": Failed to resolve address.");
        ...
        if (!couldBindToOne)
          throw Wt::WException(std::string("Could not bind to address ") + address + " port " + port + ": Could not listen on address.");
      }
      else
      {
   ...
   */
    ...
    {
      socket->async_connect(asio::ip::tcp::endpoint(asio::ip::address_v4::loopback(),
                                                    config_.parentPort()),
    ...
    void Server::stop()
    {
      ...
      wt_.ioService().post(accept_strand_.wrap(std::bind(&Server::handleStop, this)));
    }
