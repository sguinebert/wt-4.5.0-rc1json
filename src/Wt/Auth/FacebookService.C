  ...
  namespace Auth
  {
    ...
    public:
      ...
      virtual void getIdentity(const OAuthAccessToken &token) override
      {
        ...
        httpClient_->done().connect(this, std::bind(&FacebookProcess::handleMe, this,
                                                    std::placeholders::_1,
                                                    std::placeholders::_2));
        httpClient_->get("https://graph.facebook.com/me?fields=name,id,email&access_token=" + token.value());
    ...
    std::unique_ptr<OAuthProcess> FacebookService ::createProcess(const std::string &scope) const
    {
