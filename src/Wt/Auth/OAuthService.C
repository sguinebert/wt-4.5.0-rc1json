...
namespace Wt
{
  ...
  namespace Auth
  {
    ...
    void OAuthProcess::requestToken(const std::string &authorizationCode)
    {
      ...
      httpClient_->done().connect(this, std::bind(&OAuthProcess::handleToken, this,
    ...
    OAuthAccessToken OAuthProcess::parseUrlEncodedToken(const Http::Message &response)
    {
      ...
      {
        const std::string *accessTokenE = Http::Utils::getParamValue(params, "access_token");
        if (accessTokenE)
        {
          std::string accessToken = *accessTokenE;

          WDateTime expires;
          const std::string *expiresE = Http::Utils::getParamValue(params, "expires");
          if (expiresE)
            expires = WDateTime::currentDateTime().addSecs(Wt::Utils::stoi(*expiresE));
      ...
      else
      {
        if (response.status() == 200)
        {
          try
          {
            ...
            std::string refreshToken = root["refresh_token"].get_string(""); // root.get("refresh_token").orIfNull("");
            std::string idToken = root["id_token"].get_string("");
        ...
        {
          throw TokenError(ERROR_MSG(+(root["error"].get_string("missing error"))));
        }
      }
    }
