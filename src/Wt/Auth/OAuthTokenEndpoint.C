...
void OAuthTokenEndpoint::handleRequest(const Http::Request &request, Http::Response &response)
{
  ...
  if (!authCode.checkValid() || authCode.redirectUri() != *redirectUri
    ...
    LOG_INFO("{\"error\": \"invalid_grant\"}: id: {} code: {} authCode: {} redirectUri: {} timestamp: {}{}{}",
             ...
             *redirectUri,
             (authCode.redirectUri() != *redirectUri ? " - invalid" : " - valid"),
             authCode.expirationTime().toString().toUTF8(),
             (WDateTime::currentDateTime() > authCode.expirationTime() ? ", expired" : ", not expired"));
    return;
  }
