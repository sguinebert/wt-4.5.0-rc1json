...
namespace Wt
{
  namespace Dbo
  {
    ...
    Query<ptr<C>, BindStrategy> Session::find(const std::string &where)
    {
      ...
      return Query<ptr<C>, BindStrategy>(*this, '"' + Impl::quoteSchemaDot(tableName<C>()) + '"', where);
    }
    ...
    void Session::implDelete(MetaDbo<C> &dbo)
    {
      ...
      SqlStatement *statement = getStatement<C>(versioned ? SqlDeleteVersioned : SqlDelete);
    ...
    template <class C>
    void Session::Mapping<C>::dropTable(Session &session, std::set<std::string> &tablesDropped)
    {
