  ...
  namespace Dbo
  {
    ...
    namespace backend
    {
      ...
      public:
        ...
        MySQL_impl() : mysql(nullptr)
        {
        }

        MySQL_impl(MYSQL *newmysql) : mysql(newmysql)
        {
        }
      ...
      public:
              ...
              sql_(sql)
        {
          ...
          {
            throw MySQLException("error creating prepared statement: '" + sql + "': " + mysql_stmt_error(stmt_));
          }
                          ...
                          SqlDateTimeType type) override
        {
          ...
          LOG_DEBUG("{} bind {} {}", this, column, mbstr);
        ...
        {
          if (has_truncation_ && *out_pars_[column].error)
            throw MySQLException("MySQL: getResult(): truncated result for column " + std::to_string(column));
          ...
          default:
            ...
            throw MySQLException("MySQL: getResult(long long): unknown type: " + std::to_string(out_pars_[column].buffer_type));
            break;
          }
                               ...
                               SqlDateTimeType type) override
        {
          ...
          std::time_t t = std::chrono::system_clock::to_time_t(*value);
          LOG_DEBUG("{} result time {} {}", this, column, std::ctime(&t));
        ...
        virtual bool getResult(int column, std::chrono::duration<int, std::milli> *value) override
        {
          ...
          auto absValue = std::chrono::hours(ts->hour) + std::chrono::minutes(ts->minute) + std::chrono::seconds(ts->second) + msecs;
                               ...
                               int size) override
        {
          ...
          if (*(out_pars_[column].length) > 0)
          {
            ...
            LOG_DEBUG("{} result blob {} (blob, size ={})", this, column, static_cast<long long>(vlength));
                          ...
                          unsigned int dbport, const std::string &dbsocket)
      {
        ...
        if ((impl_->mysql = mysql_init(nullptr)))
        {
                                 ...
                                 CLIENT_FOUND_ROWS) != impl_->mysql)
          {
            ...
            impl_->mysql = nullptr;
            throw MySQLException(
                std::string("MySQL : Failed to connect to database server: ") + errtext);
          }
          else
          {
            // success!
