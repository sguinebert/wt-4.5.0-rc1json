...
namespace Wt
{
    ...
    namespace Impl
    {
      ...
      struct MetaDboBaseSet : public boost::multi_index::multi_index_container<
                                  ...
                                  boost::multi_index::indexed_by<
                                      ...
                                      boost::multi_index::hashed_unique<boost::multi_index::identity<MetaDboBase *>>>>
      {
      };
      ...
      std::string MappingInfo::primaryKeys(bool onlyDefault) const
      {
        ...
        else
        {
          ...
          bool firstField = true;
          for (unsigned i = 0; i < fields.size(); ++i)
            if (fields[i].isIdField() && fields[i].isDefault())
            { //&& fields[i].isDefault()
              if (!firstField)
                result << ",";
              result << "\"" << fields[i].name() << "\"";
   ...
   */
      ...
      if (!mapping->surrogateIdFieldName)
      {
        ...
        if (firstField)
          throw Exception("Table " + std::string(mapping->tableName) + " is missing a natural id defined with Wt::Dbo::id()");
      }
      else
      {
   ...
   */
      for (unsigned i = 0; i < mapping->sets.size(); ++i)
      {
        ...
        case ManyToMany:
          ...
          std::vector<JoinId> otherJoinIds = getJoinIds(otherMapping, info.joinOtherId, info.flags & Impl::SetInfo::LiteralOtherId);
          ...
          std::vector<JoinId> selfJoinIds = getJoinIds(mapping, info.joinSelfId, info.flags & Impl::SetInfo::LiteralSelfId);
...
*/
                                          ...
                                          unsigned toIndex)
    {
      ...
      if (field.fkConstraints() & Impl::FKOnUpdateCascade && haveSupportUpdateCascade_)
        sql << " on update cascade";
      else if (field.fkConstraints() & Impl::FKOnUpdateSetNull && haveSupportUpdateCascade_)
        sql << " on update set null";
      else if (field.fkConstraints() & Impl::FKOnUpdateRestrict && haveSupportUpdateCascade_)
    ...
    Session::getJoinIds(Impl::MappingInfo *mapping, const std::string &joinId, bool literalJoinId)
    {
      ...
      if (mapping->surrogateIdFieldName)
      {
        ...
        if (literalJoinId)
          ...
          idName = foreignKeyName + "_" + mapping->surrogateIdFieldName;

        result.push_back(JoinId(idName, mapping->surrogateIdFieldName, longlongType_));
      }
      else
      {
        ...
        if (literalJoinId && nbNaturalIdFields != 1)
        {
          throw Exception(std::string("The literal join id >") + joinId + " was used,"
                                                                          " but there are " +
                          std::to_string(nbNaturalIdFields) +
                                     ...
                                     bool literalJoinId)
    {
      ...
      for (unsigned i = 0; i < joinIds.size(); ++i)
        result.fields.push_back(FieldInfo(joinIds[i].joinIdName, &typeid(long long),
                                          joinIds[i].sqlType,
