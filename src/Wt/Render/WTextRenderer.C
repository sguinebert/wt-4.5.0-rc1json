...
namespace Wt
{
  ...
  namespace Render
  {
    ...
    WTextRenderer::Node::Node(Block &block, LayoutBox &lb,
                              WTextRenderer &renderer) : renderer_(renderer),
                                                         block_(block),
                                                         lb_(lb)
    {
    }
    ...
    double WTextRenderer::render(const WString &text, double y)
    {
      ...
      try
      {
        ...
        for (int i = 0; i < 2; ++i)
        {
          ...
          collapseMarginBottom = docBlock.layoutBlock(currentPs, false, *this,
          ...
          if (isEpsilonMore(currentPs.maxX, maxX))
          {
            if (!tooWide)
            {
              LOG_WARN("contents too wide for page. ({} > {})", currentPs.maxX, maxX);
              tooWide = true;
            }
