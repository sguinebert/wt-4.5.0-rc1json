...
namespace Wt
{
  ...
  namespace Render
  {
    ...
    {
      return (node_ && children_.empty() && type_ == DomElementType::UNKNOWN) || type_ == DomElementType::LI;
    }
      ...
      if (!allChildrenInline)
      {
        ...
        for (unsigned i = 0; i <= children_.size(); ++i)
        {
          ...
          if (!b || !b->isFloat())
          {
            ...
            if ((!b || !b->inline_) && firstToGroup != -1 && (int)i > firstToGroup - 1)
            {
      ...
      default:
        if (!isFloat())
        {
          ...
          if (!display.empty())
            ...
            {
              LOG_ERROR("display '{}' is not supported", display);
              inline_ = false;
            }
          else
          ...
          if (inline_ && !allChildrenInline)
            LOG_ERROR("inline element {} cannot contain block elements", DomElement::tagName(type_));
        }
        else
          inline_ = false;
      }
    ...
    {
      if (type_ == DomElementType::TABLE || type_ == DomElementType::TBODY || type_ == DomElementType::THEAD || type_ == DomElementType::TFOOT)
      {
        for (unsigned i = 0; i < children_.size(); ++i)
        {
          Block *c = children_[i];
    ...
    double Block::cssMargin(Side side, double fontScale) const
    {
      ...
      if (!result.defined)
      {
        ...
        {
          if (type_ == DomElementType::H4 || type_ == DomElementType::P
              /* || type_ == DomElementType::BLOCKQUOTE */
              || type_ == DomElementType::FIELDSET || type_ == DomElementType::FORM
              /* || type_ == DomElementType::DL */
              /* || type_ == DomElementType::DIR */
              /* || type_ == DomElementType::MENU */)
            return 1.12 * cssFontSize(fontScale);
          else if (type_ == DomElementType::UL || type_ == DomElementType::OL)
          {
                                    ...
                                    bool indicateHidden) const
    {
      ...
      int index = sideToIndex(side);
      Property property = (Property)(static_cast<unsigned int>(Property::StyleBorderTop) + index);
      ...
      {
        property = (Property)(static_cast<unsigned int>(Property::StyleBorderWidthTop) + index);
    ...
    WColor Block::rawCssBorderColor(Side side) const
    {
      ...
      {
        property = (Property)(static_cast<unsigned int>(Property::StyleBorderColorTop) + index);
    ...
    {
      return cssPadding(side, fontScale) + cssMargin(side, fontScale) + cssBorderWidth(side, fontScale);
    }
    ...
    int Block::cssFontWeight() const
    {
      ...
      if (v.empty() && (type_ == DomElementType::B || type_ == DomElementType::STRONG || type_ == DomElementType::TH || (type_ >= DomElementType::H1 && type_ <= DomElementType::H6)))
        v = "bolder";
      ...
      if (isText() || type_ == DomElementType::IMG || type_ == DomElementType::BR)
      {
        ...
        for (;;)
        {
          ...
          if (isText())
          {
             ...
             */
            if (utf8Pos < s.length() && line.x() == rangeX.start && isWhitespace(s[utf8Pos]))
              ++utf8Pos;
            ...
            if (utf8Pos < s.length())
            {
              ...
              WTextItem item = renderer.painter()->device()->measureText(text, maxWidth, true);
               ...
               */
              if (utf8Count > 0 && utf8Pos + utf8Count < s.length() && isWhitespace(s[utf8Pos + utf8Count - 1]))
              {
                w += whitespaceWidth;
          ...
          { // type_ == DomElementType::IMG
            ...
            w += cssBoxMargin(Side::Left, renderer.fontScale()) + cssBoxMargin(Side::Right, renderer.fontScale());
            h += cssBoxMargin(Side::Top, renderer.fontScale()) + cssBoxMargin(Side::Bottom, renderer.fontScale());
                            ...
                            double cssSetWidth)
    {
      ...
      for (;;)
      {
        ...
        double border = cssBorderWidth(Side::Left, renderer.fontScale()) + cssBorderWidth(Side::Right, renderer.fontScale());
      ...
      if (width > totalMaxWidth)
      {
        ...
        { /* degenerate case: all columns have width 0 */
                              ...
                              const WTextRenderer &renderer)
    {
      ...
      else if (type_ == DomElementType::TR)
      {
        ...
        double rowHeight = (ps.page - startPage) * renderer.textHeight(ps.page) // XXX
                                 ...
                                 double rowHeight)
    {
      ...
      for (unsigned i = 0; i < rowSpanBackLog.size(); ++i)
      {
        if (rowSpanBackLog[i].lastRow == cellRow_)
        {
          ...
          if (rh >= 0)
            rh += (ps.page - cs.page) * renderer.textHeight(cs.page) // XXX
                  + (ps.y - cs.y);
                                         ...
                                         Block *table)
    {
       ...
       */
      if (type_ == DomElementType::TBODY || type_ == DomElementType::THEAD || type_ == DomElementType::TFOOT)
      {
        for (unsigned i = 0; i < children_.size(); ++i)
        {
          Block *c = children_[i];
    ...
    {
      return !length.empty() && WLength(length.c_str()).unit() == LengthUnit::Percentage;
    }
                              ...
                              double cellHeight)
    {
      ...
      if (isTable())
      {
        ...
        if (cssSetWidth > 0)
        {
          cssSetWidth -= cssBorderWidth(Side::Left, renderer.fontScale()) + cssBorderWidth(Side::Right, renderer.fontScale());
      ...
      else
      {
        ...
        bool paddingBorderWithinWidth = isTableCell() && isPercentageLength(cssProperty(Property::StyleWidth));

        if (width >= 0)
        {
          if (!paddingBorderWithinWidth)
            width += cssPadding(Side::Left, renderer.fontScale()) + cssBorderWidth(Side::Left, renderer.fontScale()) + cssPadding(Side::Right, renderer.fontScale()) + cssBorderWidth(Side::Right, renderer.fontScale());
        ...
        else
        {
          ...
          double cMinX = ps.minX + cssPadding(Side::Left, renderer.fontScale()) + borderFactor * cssBorderWidth(Side::Left, renderer.fontScale());
          double cMaxX = ps.maxX - cssPadding(Side::Right, renderer.fontScale()) - borderFactor * cssBorderWidth(Side::Right, renderer.fontScale());
          ...
          else
          {
            ...
            for (unsigned i = 0; i < children_.size(); ++i)
            {
              ...
              else
              {
                ...
                collapseMarginBottom = c->layoutBlock(ps, canIncreaseWidth,
                                                      renderer,
          ...
          ps.maxX = cMaxX + cssPadding(Side::Right, renderer.fontScale()) + borderFactor * cssBorderWidth(Side::Right, renderer.fontScale());
       ...
       */
      if (!isTableCell() && (ps.maxX - ps.minX == cssSetWidth) && isPercentageLength(cssProperty(Property::StyleWidth)))
        ps.maxX = origMaxX;
    ...
    void Block::layoutAbsolute(const WTextRenderer &renderer)
    {
       ...
       */
      staticTop += (staticLayout.page - containingLayout.page) * renderer.textHeight(containingLayout.page);
                              ...
                              const WTextRenderer &renderer)
    {
      ...
      double currentWidth = std::max(0.0, blockCssWidth) + cssBoxMargin(Side::Left, renderer.fontScale()) + cssBoxMargin(Side::Right, renderer.fontScale());
    ...
    WFont Block::cssFont(double fontScale) const
    {
      ...
      if (!family.empty())
      {
        ...
        for (unsigned i = 0; i < values.size(); ++i)
        {
          ...
          {
            if (name == "times" || name == "palatino")
              genericFamily = FontFamily::Serif;
            else if (name == "arial" || name == "helvetica")
              genericFamily = FontFamily::SansSerif;
                             ...
                             LayoutBox &lb)
    {
      if (type_ == DomElementType::IMG)
      {
        ...
        double left = renderer.margin(Side::Left) + bb.x + cssBorderWidth(Side::Left, renderer.fontScale());
        double top = renderer.margin(Side::Top) + bb.y + cssBorderWidth(Side::Top, renderer.fontScale());
                           ...
                           WPainter &painter, int page)
    {
      ...
      for (unsigned i = 0; i < inlineLayout.size(); ++i)
      {
        ...
        if (ib.page == page)
        {
          double y = renderer.margin(Side::Top) + ib.y - metrics.leading() + (lineHeight - fontHeight) / 2.0;
          ...
          else
          {
            ...
            for (int j = 0; j <= ib.utf8Count; ++j)
            {
              if (j == ib.utf8Count || isWhitespace(text[ib.utf8Pos + j]))
              {
                if (j > wordStart)
                {
                  WString word = WString::fromUTF8(text.substr(ib.utf8Pos + wordStart, j - wordStart));
                  double wordWidth = device->measureText(word).width();
    ...
    {
      return cssProperty == "margin" || cssProperty == "border" || cssProperty == "padding" || cssProperty == "border-color" || cssProperty == "border-width";
    }
    ...
    {
      if (css_.find(property + aggregate) == css_.end() || css_[property + aggregate].s_.isSmallerOrEqualThen(spec))
    ...
    std::string Block::cssProperty(Property property) const
    {
      ...
      if (css_.empty())
      {
        if (styleSheet_)
        {
          for (unsigned int i = 0; i < styleSheet_->rulesetSize(); ++i)
          {
            ...
            if (s.isValid())
            {
              fillinStyle(styleSheet_->rulesetAt(i).declarationBlock().declarationString(),
                          s);
            }
          }
        }
      ...
      std::map<std::string, PropertyValue>::const_iterator i = css_.find(DomElement::cssName(property));
    ...
    {
      LOG_ERROR("unsupported value '{}' for attribute {}", value, attribute);
    }

    void Block::unsupportedCssValue(Property property, const std::string &value)
    {
      LOG_ERROR("unsupported value '{}' for CSS style property {}", value, DomElement::cssName(property));
    }
