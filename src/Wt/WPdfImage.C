  ...
  void WPdfImage::setChanged(WFlags<PainterChangeFlag> flags)
  {
    ...
    if (flags.test(PainterChangeFlag::Font))
    {
      ...
      if (!ttfFont.empty())
      {
        ...
        std::map<std::string, const char *>::const_iterator i = ttfFonts_.find(ttfFont);
        ...
        {
          std::string suffix = Utils::lowerCase(ttfFont.substr(ttfFont.length() - 4));
...
#define HAVE_LOAD_FROM_MEM HPDF_MAJOR_VERSION > 2 || (HPDF_MAJOR_VERSION == 2 && (HPDF_MINOR_VERSION >= 2))
    ...
    if (srect.x() != 0 || srect.y() != 0 || srect.width() != imgWidth || srect.height() != imgHeight)
    {
  ...
  void WPdfImage::drawPlainPath(const WPainterPath &path)
  {
    ...
    if (segments.size() > 0 && segments[0].type() != SegmentType::MoveTo)
                           ...
                           const WPointF *clipPoint)
  {
    ...
    {
      if (!painter()->clipPathTransform().map(painter()->clipPath()).isPointInPath(painter()->worldTransform().map(*clipPoint)))
        return;
    }
