...
namespace Wt
{
  ...
  namespace Http
  {
    ...
    public:
      ...
      {
        ioService_.post(strand_.wrap(std::bind(&Impl::stop, shared_from_this(), impl)));
      }
    ...
    private:
      ...
      void startTimer()
      {
        ...
        timer_.async_wait(strand_.wrap(std::bind(&Impl::timeout, shared_from_this(),
                                                 std::placeholders::_1)));
      }
                         ...
                         tcp::resolver::iterator endpoint_iterator)
      {
        ...
        if (!err && !aborted_)
        {
          ...
          startTimer();
          asyncHandshake(strand_.wrap(std::bind(&Impl::handleHandshake,
                                                shared_from_this(),
                                                std::placeholders::_1)));
        }
      ...
      void handleHandshake(const AsioWrapper::error_code &err)
      {
        ...
        if (!err && !aborted_)
        {
          ...
          startTimer();
          asyncWriteRequest(strand_.wrap(std::bind(&Impl::handleWriteRequest,
                                                   shared_from_this(),
                                                   std::placeholders::_1,
                                                   std::placeholders::_2)));
        }
                              ...
                              const std::size_t &)
      {
        ...
        if (!err && !aborted_)
        {
          ...
          startTimer();
          asyncReadUntil("\r\n",
                         strand_.wrap(std::bind(&Impl::handleReadStatusLine,
                                                shared_from_this(),
                                                std::placeholders::_1,
                                                std::placeholders::_2)));
        }
                                ...
                                const std::size_t &s)
      {
        ...
        if (!err && !aborted_)
        {
          ...
          if (!response_stream || http_version.substr(0, 5) != "HTTP/")
          {
            ...
            err_ = boost::system::errc::make_error_code(boost::system::errc::protocol_error);
#else
          ...
          startTimer();
          asyncReadUntil("\r\n\r\n",
                         strand_.wrap(std::bind(&Impl::handleReadHeaders,
                                                shared_from_this(),
                                                std::placeholders::_1,
                                                std::placeholders::_2)));
        }
                             ...
                             const std::size_t &s)
      {
        ...
        if (!err && !aborted_)
        {
          ...
          if (!done)
          {
            ...
            startTimer();
            asyncRead(strand_.wrap(std::bind(&Impl::handleReadContent,
                                             shared_from_this(),
                                             std::placeholders::_1,
                                             std::placeholders::_2)));
          }
                             ...
                             const std::size_t &s)
      {
        ...
        else if (!aborted_ && err != asio::error::eof && err != asio::error::shut_down && err != asio::error::bad_descriptor && err != asio::error::operation_aborted && err.value() != 335544539)
        {
          err_ = err;
          complete();
        }
      ...
      void chunkedDecode(const std::string &text)
      {
        ...
        while (pos != text.end())
        {
          ...
          {
            std::size_t thisChunk = std::min(std::size_t(text.end() - pos), chunkState_.size);
